...
lui指令仅对立即数处理，因此在ID阶段就可以得到结果，在EX阶段将其并入R型指令的数据通路。

##### （2）jal指令

jal指令特殊处理，可以当做在EX阶段才能得到结果的指令，并将数据并入R型运算指令的数据流中以简化设计。

##### （3）阻塞方法

$T_{new}>T_{use}$ 时，说明数据不能在使用的时刻前产生，需要阻塞。阻塞时，冻结IF/ID和PC寄存器，同步清零ID/EX寄存器，并更新前一指令的 $T_{new}$ 值。
...
ID/EX寄存器，对应于在ID阶段可以产生结果的指令；

EX/Mem寄存器，对应于在EX阶段可以产生结果的指令；

Mem/WB寄存器，对应于在Mem阶段可以产生结果的指令。

转发需求：

ID/EX寄存器入口，与GRF读出数据合并进行选择；
...
每级流水中传下来的Rs，Rt的地址，若非0，则为需要读取且需要注意冲突的寄存器地址。每级流水中的Rd（后面改名为RegWriteAddr）若非0则为最终要写的寄存器地址。
